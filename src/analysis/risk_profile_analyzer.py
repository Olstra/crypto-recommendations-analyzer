...
def enrich_with_variables(df: DataFrame) -> DataFrame:
    """Parses risk profiles from JSON variables or prompt text."""
    ...
    def parse(row) -> Series:
        ...
        return Series([str(risk).lower().strip()])

    df[["risk_profile"]] = df.apply(parse, axis=1)
    return df
...
def run_analysis_block(
    ...
    if cleaned_data.empty:
        return ["No data available for this block."]
...
def generate_visualizations(
    ...
    """Generates the unified 5-in-1 dashboard image for the risk profile."""
    if df.empty:
        return
    ...
    plt.suptitle(
        f"Risk Profile Analysis: {group_name.title()}",
        fontsize=18,
        fontweight="bold",
        y=0.97,
    )
    ...
    dashboard_path = output_dir / f"risk_{group_clean}_dashboard_top_{top_k}.png"
    plt.savefig(dashboard_path, dpi=300, bbox_inches="tight")
    plt.close()

    logger.info(
        f"Generated Risk Profile Dashboard for {group_name} at: {dashboard_path}"
    )
...
def main(input_data: Path, top_k: int):
    analysis_out_dir = OUTPUT_PATH_ANALYSIS / "risk_profile_analysis"
    analysis_out_dir.mkdir(parents=True, exist_ok=True)
    report_file = analysis_out_dir / "01_risk_profile_analysis.txt"

    logger.info("Loading and enriching data for Risk Profile analysis...")
    ...
    lines = [
        "===========================================================",
        " RISK PROFILE ANALYSIS REPORT",
        "===========================================================\n",
        f"Total Records: {len(df)} | Evaluated Models: {len(df['model'].unique())}\n",
    ]

    df_seeking = df[df["risk_profile"] == "risk-seeking"].copy()
    df_averse = df[df["risk_profile"] == "risk-averse"].copy()

    df_seeking.to_csv(analysis_out_dir / "risk_seeking_subset.csv", index=False)
    df_averse.to_csv(analysis_out_dir / "risk_averse_subset.csv", index=False)

    lines.extend(["\n" + "=" * 50, " RISK-SEEKING PROFILE", "=" * 50])
    lines.extend(run_analysis_block(df_seeking, top_k=top_k, exclude=EXCLUDED_TOKENS))

    lines.extend(["\n\n" + "=" * 50, " RISK-AVERSE PROFILE", "=" * 50])
    lines.extend(run_analysis_block(df_averse, top_k=top_k, exclude=EXCLUDED_TOKENS))

    generate_visualizations(
        df_seeking, "Risk-Seeking", analysis_out_dir, top_k, EXCLUDED_TOKENS
    )
    generate_visualizations(
        df_averse, "Risk-Averse", analysis_out_dir, top_k, EXCLUDED_TOKENS
    )

    with open(report_file, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))

    logger.info(f"Success! Risk Profile analysis report saved to: {report_file}")
